...
// ── Wizard state ─────────────────────────────────────────────────────────────
const step = ref(1) // 1=welcome 2=requirements 3=database 4=site 5=admin 6=installing 7=done
...
async function testDb() {
  ...
  try {
    const { data } = await axios.post('/install/test-database', db)
    dbStatus.value = data.success ? { ok: true, version: data.version } : { ok: false, error: data.error }
  } catch (e) {
  ...
  <div class="min-h-screen bg-gray-950 flex flex-col items-center justify-center px-4 py-12">
    ...
    <div class="w-full max-w-lg mb-8">
      <div class="flex justify-between text-xs text-gray-500 mb-1.5">
        <span v-for="(label, i) in ['Welcome','Requirements','Database','Site','Admin','Complete']"
          :key="i"
    ...
    <div class="w-full max-w-lg bg-gray-900 rounded-2xl border border-gray-800 shadow-2xl p-8">
      ...
      <div v-else-if="step === 2">
        ...
        <div v-else class="space-y-2 mb-6">
          <div v-for="(check, key) in reqs.checks" :key="key"
            ...
            :class="check.pass ? 'bg-green-900/20 border border-green-800/50' : 'bg-red-900/20 border border-red-800/50'"
          >
            <span :class="check.pass ? 'text-green-300' : 'text-red-300'">{{ check.label }}</span>
            <span class="text-xs font-mono" :class="check.pass ? 'text-green-500' : 'text-red-500'">
      ...
      <div v-else-if="step === 3">
        ...
        <div class="flex gap-3">
          ...
          <button @click="goToSite" :disabled="!dbStatus?.ok"
            class="flex-1 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white font-medium rounded-lg py-2.5 text-sm transition-colors">
            Continue →
          </button>
        </div>
      ...
      <div v-else-if="step === 4">
        ...
        <div class="flex gap-3">
          ...
          <button @click="step = 5" :disabled="!site.name || !site.url"
            class="flex-1 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white font-medium rounded-lg py-2.5 text-sm transition-colors">
            Continue →
          </button>
        </div>
      ...
      <!-- Step 6: Installing -->
      <div v-else-if="step === 6" class="text-center py-8">
      ...
      <div v-else-if="step === 7" class="text-center py-4">
        ...
        <a :href="site.url + '/login'"
          class="inline-block bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-lg px-6 py-2.5 text-sm transition-colors">
          Go to Login →
        </a>
      </div>
