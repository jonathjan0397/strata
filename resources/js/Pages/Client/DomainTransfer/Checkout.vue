...
const creditApplied = computed(() =>
    form.apply_credit ? Math.min(props.creditBalance, props.price) : 0
)

const total = computed(() => Math.max(0, props.price - creditApplied.value).toFixed(2))

function place() {
    form.post(route('client.domain-transfer.place'))
}
</script>
...
<template>
  <div class="max-w-3xl mx-auto">
    <div class="mb-6">
      <Link :href="route('client.domain-transfer.search')" class="text-xs text-gray-400 hover:text-gray-600">← Back</Link>
      <h1 class="mt-2 text-xl font-bold text-gray-900">Transfer {{ domain }}</h1>
    </div>
    ...
    <form @submit.prevent="place" class="space-y-6">
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div class="flex items-center justify-between text-sm mb-4">
          ...
          <span class="text-gray-500 text-xs">Transfer (1 year renewal included)</span>
        </div>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <p class="text-xs text-gray-500 mb-4">Contact information to be associated with the domain after transfer.</p>
      ...
      <div class="flex items-center justify-between">
        <p class="text-xs text-gray-500">
          Transfer is initiated once your invoice is paid. Completion typically takes 5–7 days.
        </p>
