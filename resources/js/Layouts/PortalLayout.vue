...
<template>
  ...
  <div class="min-h-screen relative overflow-x-hidden"
    style="background: linear-gradient(135deg, #0c1445 0%, #1a3a6b 40%, #0e7490 80%, #0891b2 100%);">
    ...
    <header class="sticky top-0 z-50"
      style="background: rgba(12,20,69,0.55); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border-bottom: 1px solid rgba(255,255,255,0.1);">
        ...
        <div class="ml-auto flex items-center gap-3">
          <template v-if="user">
            <Link :href="user.roles?.some(r => ['super-admin','admin','staff'].includes(r.name)) ? route('admin.dashboard') : route('client.dashboard')"
              ...
              style="background: rgba(56,189,248,0.2); border: 1px solid rgba(56,189,248,0.35);"
              onmouseover="this.style.background='rgba(56,189,248,0.35)'"
              onmouseout="this.style.background='rgba(56,189,248,0.2)'">
              My Portal
            </Link>
          </template>
          <template v-else>
            ...
            <Link :href="route('register')"
              ...
              onmouseout="this.style.boxShadow='0 2px 12px rgba(14,165,233,0.35)'">
              Get Started
            </Link>
          </template>
      ...
      <div v-if="mobileOpen" class="md:hidden border-t px-4 py-3 space-y-1"
        style="border-color: rgba(255,255,255,0.1); background: rgba(12,20,69,0.8);">
        <a v-for="n in nav" :key="n.label" :href="n.href"
