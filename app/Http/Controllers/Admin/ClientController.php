...
class ClientController extends Controller
{
    ...
    public function sendEmail(Request $request, User $client): RedirectResponse
    {
        ...
        $mailer = \App\Models\Setting::get('mail_mailer', config('mail.default'));
        $from   = \App\Models\Setting::get('mail_from_address', config('mail.from.address'));

        if ($mailer === 'sendmail') {
            // Bypass Laravel's transport (hangs); use proc_open directly like testMail
            $path = \App\Models\Setting::get('mail_sendmail_path', '/usr/sbin/sendmail -t -i');
            $bin  = explode(' ', $path)[0];
